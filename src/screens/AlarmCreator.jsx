...
import { scheduleNotification } from '../../api/notification';
import { getNextTime } from '../../api/Utils';
...
const AlarmCreator = ({ route, navigation }) => {
    ...
    const addAlarm = () => {
        const days = getDay()
        const newAlarm = { id: 0, hour: hour, minute: minute, music: 1, vibrate: 0, days }
        //console.log({newAlarm}, "checking here now")
        
        Database.add(newAlarm.hour, newAlarm.minute, newAlarm.music, newAlarm.vibrate, newAlarm.days)
        //console.log(`added a new alarm at ${newAlarm.days}:${newAlarm.hour}:${newAlarm.minute}`)
        
        const time = getNextTime(days[0]['name'], hour, minute)
